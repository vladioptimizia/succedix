    ...
    <main className="min-h-screen flex flex-col">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-2">
          <Link href="/onboarding/buyer" className="h-12 px-8 rounded-full font-medium text-sm flex items-center justify-center gap-2 transition-all" style={{ background: '#10b981', color: '#fff', boxShadow: '0 0 24px rgba(16,185,129,0.3)' }}>
            {t.hero.ctaBuy}
          </Link>
          <Link href="/onboarding/seller" className="h-12 px-8 rounded-full font-medium text-sm flex items-center justify-center gap-2 transition-all" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#e5e7eb' }}>
            {t.hero.ctaSell}
          </Link>
        </div>
      ...
      <section className="px-6 py-24" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-center text-gray-500 mb-16 max-w-xl mx-auto">{t.pillars.subtitle}</p>
          <div className="grid md:grid-cols-3 gap-6">
            <PillarCard icon="🔒" title={t.pillars.discretion.title} description={t.pillars.discretion.quote} features={[t.pillars.discretion.f1, t.pillars.discretion.f2, t.pillars.discretion.f3, t.pillars.discretion.f4]} />
            <PillarCard icon="🎯" title={t.pillars.matching.title} description={t.pillars.matching.quote} features={[t.pillars.matching.f1, t.pillars.matching.f2, t.pillars.matching.f3, t.pillars.matching.f4]} highlight />
            <PillarCard icon="🚀" title={t.pillars.support.title} description={t.pillars.support.quote} features={[t.pillars.support.f1, t.pillars.support.f2, t.pillars.support.f3, t.pillars.support.f4]} />
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-20" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-6">
          <AudienceCard tag={t.sellers.tag} headline={t.sellers.title} body={t.sellers.desc} points={[t.sellers.f1, t.sellers.f2, t.sellers.f3, t.sellers.f4]} cta={t.sellers.cta} href="/onboarding/seller" />
          <AudienceCard tag={t.buyers.tag} headline={t.buyers.title} body={t.buyers.desc} points={[t.buyers.f1, t.buyers.f2, t.buyers.f3, t.buyers.f4]} cta={t.buyers.cta} href="/onboarding/buyer" />
        </div>
      </section>
      ...
      <section className="px-6 py-24 flex flex-col items-center text-center gap-6" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link href="/onboarding/buyer" className="h-12 px-10 rounded-full font-medium text-sm flex items-center justify-center" style={{ background: '#10b981', color: '#fff', boxShadow: '0 0 32px rgba(16,185,129,0.25)' }}>
            {t.cta.ctaBuy}
          </Link>
          <Link href="/onboarding/seller" className="h-12 px-8 rounded-full font-medium text-sm flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#e5e7eb' }}>
            {t.cta.ctaSell}
          </Link>
        </div>
      </section>
...
function PillarCard({ icon, title, description, features, highlight = false }: { icon: string; title: string; description: string; features: string[]; highlight?: boolean }) {
  return (
    <div className="p-8 rounded-2xl flex flex-col gap-4" style={{ background: highlight ? 'rgba(16,185,129,0.06)' : 'rgba(255,255,255,0.02)', border: highlight ? '1px solid rgba(16,185,129,0.25)' : '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <ul className="space-y-2 mt-2">
        {features.map((f, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-gray-400">
            <span className="mt-0.5 text-success flex-shrink-0">✓</span>{f}
          </li>
        ))}
      </ul>
    </div>
...
function StepCard({ number, title, description, accent = false }: { number: string; title: string; description: string; accent?: boolean }) {
  return (
    <div className="p-6 rounded-2xl flex flex-col gap-3" style={{ background: accent ? 'rgba(16,185,129,0.06)' : 'rgba(255,255,255,0.03)', border: accent ? '1px solid rgba(16,185,129,0.2)' : '1px solid rgba(255,255,255,0.06)' }}>
      <span className="font-serif text-3xl font-bold" style={{ background: 'linear-gradient(135deg, #10b981, #34d399)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{number}</span>
...
function AudienceCard({ tag, headline, body, points, cta, href }: { tag: string; headline: string; body: string; points: string[]; cta: string; href: string }) {
  return (
    <div className="p-8 rounded-2xl flex flex-col gap-5" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <ul className="space-y-2">
        {points.map((p, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-gray-400">
            <span className="mt-0.5 text-success flex-shrink-0">✓</span>{p}
          </li>
        ))}
      </ul>
      <Link href={href} className="mt-2 self-start h-9 px-5 rounded-full text-sm font-medium flex items-center" style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', color: '#34d399' }}>
        {cta} →
      </Link>
    </div>
  );
